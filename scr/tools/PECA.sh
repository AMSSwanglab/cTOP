#/bin/bash
#cd ~/project/lurong/16.peca/03.gumo_merge
...
macs3 callpeak -t ../../Input/${input}.bam -f BAM -n ${input} -g 2.5e9 --nomodel --shift -100 --extsize 200
cat ${input}_peaks.narrowPeak|awk 'BEGIN{FS="\t";OFS="\t"}{print $1,$2,$3,$1"_"$2"_"$3}'|grep -v rand |grep -v chrMT > region.txt

echo step 2: motif binding....
findMotifsGenome.pl region.txt /public/home/wangwen_lab/lizihe/project/lurong/06.atac/genome/mhl.primary.chr.mt.fa ./. -size given -find ../../Data/all_motif_rmdup -preparsedDir ../../Homer/ -p $numCore > MotifTarget.bed
...
cd ./Enrichment/
findMotifsGenome.pl region.bed /public/home/wangwen_lab/lizihe/project/lurong/06.atac/genome/mhl.primary.chr.mt.fa ./. -size given -mask -nomotif -mknown ../../../Data/all_motif_rmdup -preparsedDir ../../../Homer/ -p $numCore
